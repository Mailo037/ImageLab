...
type Item = { id: string; file: File; url: string; name: string; size: number; type: string; width?: number; height?: number; status: Status; progress?: number; error?: string; outputSize?: number; outputFormat?: Format };
...
type ContextState = { target: ContextTarget; x: number; y: number } | null;
type MenuAction = { id: string; label: string; icon: LucideIcon; run: () => void; disabled?: boolean; tone?: "danger" } | { id: string; separator: true };
...
function Button({ children, className = "", ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { className?: string }) { return <button className={`button ${className}`} {...props}>{children}</button>; }
function ImageLabTooltip({ label, children }: { label: string; children: ReactNode }) { return <span className="il-tooltip"><span>{children}</span><i role="tooltip">{label}</i></span>; }
function IconButton({ label, children, className = "", ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { label: string; className?: string }) { return <button aria-label={label} className={`icon-button ${className}`} {...props}>{children}</button>; }

function ImageLabSlider({ label, value, min, max, step = 1, unit = "", defaultValue, onChange, onCommit }: { label: string; value: number; min: number; max: number; step?: number; unit?: string; defaultValue?: number; onChange: (value: number) => void; onCommit: () => void }) {
...
function ImageLabNumberInput({ value, min, max, step = 1, placeholder, unit, label, defaultValue, onChange, onCommit }: { value: number | null; min?: number; max?: number; step?: number; placeholder?: string; unit?: string; label: string; defaultValue?: number | null; onChange: (value: number | null) => void; onCommit: () => void }) {
...
function ImageLabSelect({ label, value, options, onChange, disabled = false }: { label: string; value: string; options: Array<{ value: string; label: string }>; onChange: (value: string) => void; disabled?: boolean }) {
...
function ImageLabColorField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) { const picker=useRef<HTMLInputElement | null>(null);return <label className="color"><b>{label}</b><span><button type="button" className="color-swatch" style={{background:value}} aria-label={`Choose ${label}`} onClick={()=>picker.current?.click()} /><input ref={picker} className="sr-only" tabIndex={-1} type="color" value={value} onChange={(event)=>onChange(event.target.value)} /><input className="color-value" value={value.toUpperCase()} aria-label={`${label} value`} onChange={(event)=>/^#[0-9a-fA-F]{0,6}$/.test(event.target.value)&&onChange(event.target.value)} /></span></label>; }
function ImageLabSwitch({ label, help, value, onChange }: { label: string; help?: string; value: boolean; onChange: (value: boolean) => void }) { return <label className="toggle"><span><b>{label}</b>{help&&<small>{help}</small>}</span><input className="control-input" type="checkbox" checked={value} onChange={(event)=>onChange(event.target.checked)} /><i aria-hidden="true" /></label>; }
function ImageLabCheckbox({ label, checked, onChange }: { label: string; checked: boolean; onChange: () => void }) { return <label className="il-checkbox" aria-label={label} onClick={(event)=>event.stopPropagation()}><input className="control-input" type="checkbox" checked={checked} onChange={onChange}/><i aria-hidden="true"><Check size={10}/></i></label>; }
function ImageLabProgress({ value, label }: { value: number; label: string }) { return <div className="il-progress" role="progressbar" aria-label={label} aria-valuemin={0} aria-valuemax={100} aria-valuenow={value}><i style={{width:`${value}%`}}/></div>; }
function ImageLabDisclosure({label,children}:{label:string;children:ReactNode}) { const [open,setOpen]=useState(false);return <div className="advanced-disclosure"><button type="button" aria-expanded={open} onClick={()=>setOpen((value)=>!value)}><ChevronRight size={13}/>{label}</button>{open&&<div>{children}</div>}</div>; }
...
function Queue({files,active,selected,onActive,onToggle,retry,browse,onMenu}:{files:Item[];active:string;selected:string[];onActive:(id:string)=>void;onToggle:(id:string)=>void;retry:(id:string)=>void;browse:()=>void;onMenu:(id:string,x:number,y:number)=>void}) {
...
function ImageLabContextMenu({menu,items,close}:{menu:ContextState;items:MenuAction[];close:()=>void}) {
...
function ImageLabDialog({title,description,children,onClose}:{title:string;description?:string;children:ReactNode;onClose:()=>void}) {
